...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
	...
	<script type="text/javascript">
		...
		function deleteUser() {
			var username = $(clickTrObj).find('td').eq(1).text();
			if (username == '') {
				alert("请点击要删除的列");
				return;
			}
			$.ajax("<@s.url'/user/deleteUser.html?userName=" + username + "'/>");
			...
			myPagination = $("#pageBar").myPagination(
				{
					...
					panel : {
						...
						ajax : {
							...
							type : "POST",
							url : "<@s.url'/user/getJsonUserList.html'/>",
							dataType : 'json',
							param : {
								on : true,
								page : 1
								...
								$("#userTable > tbody").html(""); //每次加载的时候先设为空
									$.each(data.event,function(i, value) {
										$("#userTable > tbody").append("<tr>"
												...
												+ "<td> " + value.locked + "</td>"
												+ "<td> " + value.credit + "</td>"
												+ "<td> " + timeStamp2String(value.lastVisit) + "</td>"
												+ "<td> " + value.lastLogIp + "</td>"
												+ "<td>"
												+ "<a href=\"javascript:void(0)\" onclick=\"update(this);\" class=\"btn btn-primary btn-xs active\" role=\"button\">Update</a>&nbsp;|&nbsp;"
												+ "<a href=\"<@s.url'/user/deleteUser.html?userName=" + value.userName + "'/>\" class=\"btn btn-danger btn-xs active\" role=\"button\">Delete</a>"
												+ "</td>"
												+ "</tr>");
									});
...
<script type="text/javascript">
	...
	<div class="container">
		...
		<div class="row">
            ...
            <#--<button type="button" class="btn btn-success" data-toggle="modal"-->
                    <#--data-target="#uploadwin">UpLoadFile</button>-->
		...
		<div class="modal fade" id="addWin" tabindex="-1" role="dialog"
			...
			<div class="modal-dialog">
				<form class="form-horizontal" action="<@s.url'/user/addUser.html'/>"
					role="form">
		...
		<div class="modal fade" id="updateWin" tabindex="-1" role="dialog"
			aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<form class="form-horizontal" action="<@s.url'/user/updateUser.html'/>" role="form" method="post">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal"aria-hidden="true">&times;</button>
						...
						<div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="u_password" class="col-xs-2 control-label">UserTYpe</label>
                                <div class="col-xs-10">
                                    <input type="text" class="form-control" id="u_type" name="userType">
                                </div>
                            </div>
						</div>
						<div class="modal-footer">
